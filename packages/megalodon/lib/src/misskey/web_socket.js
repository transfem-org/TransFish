...
var WebSocket = (function (_super) {
    ...
    function WebSocket(url, channel, accessToken, listId, userAgent, proxyConfig, converter) {
    ...
    WebSocket.prototype.baseUrlToHost = function (baseUrl) {
    ...
    WebSocket.prototype._connect = function () {
        ...
        if (this.proxyConfig) {
            options = Object.assign(options, {
                agent: (0, proxy_config_1.default)(this.proxyConfig)
            });
        }
        var cli = new ws_1.default("".concat(this.url, "?i=").concat(this._accessToken), options);
        return cli;
    };
    ...
    WebSocket.prototype._bindSocket = function (client) {
        ...
        client.on('message', function (data, isBinary) {
            _this.parser.parse(data, isBinary, _this._channelID);
        });
    ...
    WebSocket.prototype._setupParser = function () {
        ...
        this.parser.on('update', function (note) {
            _this.emit('update', _this._converter.note(note, _this.baseUrlToHost(_this.url)));
        });
        this.parser.on('notification', function (notification) {
            _this.emit('notification', _this._converter.notification(notification, _this.baseUrlToHost(_this.url)));
        });
        this.parser.on('conversation', function (note) {
            _this.emit('conversation', _this._converter.noteToConversation(note, _this.baseUrlToHost(_this.url)));
        });
...
var Parser = (function (_super) {
    ...
    Parser.prototype.parse = function (data, isBinary, channelID) {
