...
<MkSpacer :content-max="1200">
	...
	<div v-if="origin === 'local'">
		<template v-if="tag == null">
			<MkFolder v-if="pinnedUsers.length > 0" class="_gap" persist-key="explore-pinned-users">
				<template #header><i class="ph-bookmark-simple ph-fw" style="margin-right: 0.5em;"></i>{{ i18n.ts.pinnedUsers }}</template>
				<XUserList :pagination="pinnedUsers"/>
			</MkFolder>
			<MkFolder class="_gap" persist-key="explore-popular-users">
				<template #header><i class="ph-chart-line-up ph-fw" style="margin-right: 0.5em;"></i>{{ i18n.ts.popularUsers }}</template>
				<XUserList :pagination="popularUsers"/>
			</MkFolder>
			<MkFolder class="_gap" persist-key="explore-recently-updated-users">
				<template #header><i class="ph-chat ph-fw" style="margin-right: 0.5em;"></i>{{ i18n.ts.recentlyUpdatedUsers }}</template>
				<XUserList :pagination="recentlyUpdatedUsers"/>
			</MkFolder>
			<MkFolder class="_gap" persist-key="explore-recently-registered-users">
				<template #header><i class="ph-plus ph-fw" style="margin-right: 0.5em;"></i>{{ i18n.ts.recentlyRegisteredUsers }}</template>
	...
	<div v-else>
		<MkFolder ref="tagsEl" :foldable="true" :expanded="false" class="_gap">
			<template #header><i class="ph-hash ph-fw" style="margin-right: 0.5em;"></i>{{ i18n.ts.popularTags }}</template>
		...
		<template v-if="tag == null">
			...
			<MkFolder class="_gap">
				<template #header><i class="ph-rocket-launch ph-fw" style="margin-right: 0.5em;"></i>{{ i18n.ts.recentlyDiscoveredUsers }}</template>
