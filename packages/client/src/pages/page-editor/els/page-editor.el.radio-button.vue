...
<XContainer :draggable="true" @remove="() => $emit('remove')">
	<template #header><i class="ph-lightning-bold"></i> {{ $ts._pages.blocks.radioButton }}</template>

	<section style="padding: 0 16px 16px 16px;">
		<MkInput v-model="value.name"><template #prefix><i class="ph-magic-wand-bold"></i></template><template #label>{{ $ts._pages.blocks._radioButton.name }}</template></MkInput>
