<template>
<div class="_formRoot">
	<FormLink to="/settings/plugin/install"><template #icon><i class="ph-download-simple-bold"></i></template>{{ i18n.ts._plugin.install }}</FormLink>
	...
	<FormSection>
		...
		<div v-for="plugin in plugins" :key="plugin.id" class="_formBlock _panel" style="padding: 20px;">
			...
			<div style="display: flex; gap: var(--margin); flex-wrap: wrap;">
				<MkButton v-if="plugin.config" inline @click="config(plugin)"><i class="ph-gear-six-bold"></i> {{ i18n.ts.settings }}</MkButton>
				<MkButton inline danger @click="uninstall(plugin)"><i class="ph-trash-bold"></i> {{ i18n.ts.uninstall }}</MkButton>
			</div>
		</div>
	</FormSection>
</div>
